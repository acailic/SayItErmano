...
def test(run) -> QualityCheckResult:
    ...
    return _run(QualityCheckSpec(
        ...
        argv=[".venv/bin/python", "-m", "pytest", "-q", "tests",
              "--ignore=tests/integration"],  # integration needs mic/GPU/daemon
        timeout_seconds=600,
    ), run)
...
def lint(run) -> QualityCheckResult:
    return _run(QualityCheckSpec(
        ...
        area="backend",
        operation="lint",
        argv=[".venv/bin/python", "-m", "compileall", "-q", "fluidvoice"],  # syntax gate; no linter configured
    ), run)
...
def typecheck(run) -> QualityCheckResult:
    return _run(QualityCheckSpec(
        ...
        area="backend",
        operation="typecheck",
        argv=[".venv/bin/python", "-m", "compileall", "-q", "tests"],  # no typechecker configured
    ), run)
...
def build(run) -> QualityCheckResult:
    ...
    return _run(QualityCheckSpec(
        ...
        area="backend",
        operation="build",
        argv=[".venv/bin/python", "-m", "pip", "wheel", ".", "--no-deps",
              "-w", str(output_dir)],  # real packaging check
    ), run)
