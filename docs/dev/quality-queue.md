...
| Q7 process/GTK/desktop lanes + matrices | **desktop-matrix X11 half: evidence 2026-09-13 (night); code half shipped 2026-09-12** | Q1, Q2 | 09e1b9f evidence run | `just test-process`, `just test-gtk`, live matrices | Integration tier green on real hardware 2026-09-13 (32 passed / 8 honest capability skips: 7 GPU-busy because the production daemon holds large-v3, 1 misleading copyq-env skip): [integration run](../research/night-2026-09-13-integration-run.md). GNOME X11 desktop matrix (Matrix 4 + Matrix C) executed live with synthetic-speech end-to-end takes: typed paths all ✓ (gedit, gnome-terminal byte-exact via AT-SPI, Firefox, Chromium, Discord), A15 focus-change ✓, absent-tools ✓, copyq hygiene ✓, privacy sweep ✓, a11y-degrade ✓; paste paths reproduced four insertion-hardening defects (terminal duplication F4, Firefox/Discord false-verified loss F5, Chromium stale-clipboard insert F6) and the atspi provider is blocked by F1–F3 (app-cap truncation, wrong-ACTIVE-window pick, GIR text-call) — see [desktop-matrix run](../research/night-2026-09-13-desktop-matrix-x11.md). STILL OPEN: GNOME Wayland + sway REQ-PARITY matrices (need live sessions), KDE, VS Code/Slack/Telegram/XFCE (absent), LibreOffice byte-exact, spoken-send live, lock/resume + USB replug cases. C3/atspi blocker trio F1–F3 (ledger F-31..F-33) FIXED 2026-09-14 in 1f26739 (mechanism live-verified on the repro desktop); the C3 cell itself awaits a matrix rerun |
